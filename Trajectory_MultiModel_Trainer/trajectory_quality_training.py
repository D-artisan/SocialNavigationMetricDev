...
def load_labels(labels_file):
    ...
    try:
        ...
        print(f"Error loading labels file {labels_file}: {e}")
        return {}
...
def train_evaluate_model(model, X_train, y_train, X_test, y_test, feature_names):
    """
    Trains and evaluates a given model.
    Returns evaluation metrics and feature importances.
    """
    ...
    print(f"{model_name} - Mean Squared Error (MSE): {mse:.6f}")
    print(f"{model_name} - R^2 Score: {r2:.6f}")
    print(f"{model_name} - Cross-Validation MSE: {cv_mse:.6f}")
    print(f"{model_name} - Top 5 Feature Importances:")
...
def main():
    ...
    if not trajectories:
        ...
        return
    if not labels:
        print("No labels data found. Exiting.")
        return
    ...
    if dataset.empty:
        print("Dataset is empty after processing. Check your data and labels.")
        return
    ...
    models = {
        ...
        'XGBRegressor': XGBRegressor(n_estimators=100, learning_rate=0.1, random_state=42, verbosity=0),
        'LGBMRegressor': lgb.LGBMRegressor(n_estimators=100, learning_rate=0.1, random_state=42)
    }

    # Initialise a list to store evaluation results
    evaluation_results = []

    # Iterate over each model, train, predict, and evaluate
    for model_name, model in models.items():
        result = train_evaluate_model(model, X_train_scaled, y_train, X_test_scaled, y_test, feature_names)
        evaluation_results.append(result)

    # Create a summary DataFrame
    summary_df = pd.DataFrame({
        'Model': [result['Model'] for result in evaluation_results],
        'Mean Squared Error (MSE)': [result['MSE'] for result in evaluation_results],
        'R^2 Score': [result['R2_Score'] for result in evaluation_results],
        'Cross-Validation MSE': [result['Cross_Validation_MSE'] for result in evaluation_results]
    })

    # Sort the DataFrame by MSE in ascending order
    summary_df.sort_values(by='Mean Squared Error (MSE)', inplace=True)
